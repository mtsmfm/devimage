...
The fix is to also mount the WSL host's d3d12 userland into the container. For OpenGL, Mesa's `d3d12` Gallium driver routes GL → DirectX 12 → Windows GPU driver. For browser WebGPU, devimage also includes a patched Mesa `dzn` Vulkan ICD under `/opt/mesa-dzn`; it is patched for the Dawn/Chromium adapter requirements that stock dzn currently misses. The `fullDrawIndexUint32` half has public WSL2/Dozen prior art ([google-gemini/gemini-cli#27182](https://github.com/google-gemini/gemini-cli/issues/27182), [Mycomembranes/adreno-wsl2-gpu](https://github.com/Mycomembranes/adreno-wsl2-gpu)); devimage also advertises binary semaphore sync features so Chrome page WebGPU can pass its external-image gate instead of falling back to SwiftShader.

Uncomment the WSL2 lines in [`compose.yml`](compose.yml), or add the equivalent flags to `docker run`:
...
For Chrome/WebGPU, use the wrapper:
...
In `chrome://gpu`, Dawn should list a Vulkan adapter similar to `Microsoft Direct3D12 (NVIDIA GeForce RTX ...)`, and page code should get a non-fallback adapter from `navigator.gpu.requestAdapter({ powerPreference: "high-performance" })`.
...
- A boot-time s6 oneshot ([`devimage-trust-proxy-ca`](scripts/devimage-trust-proxy-ca)) installs the MITM CA into the system trust store and `/config/.pki/nssdb`, so apt / git / curl / pip / Chrome transparently verify HTTPS through the proxy. No-op when the throttle overlay isn't in use; the script is also runnable manually for debugging.
